...
│   ├── 操作调研-全量汇总表.md          ← 主表（已合并旧仓库 + 链上验证）
│   ├── 操作截图/                       ← 旧仓库带过来的 9 张截图
│   ├── 优先4项-操作链接与记录表.md      ← 操作指引：BNSOL / Orca / Ember / Re
│   ├── 优先4项-交易记录表.csv           ← 🔴🔴 直接填这个（23 步）
│   ├── 交易采集-操作清单与记录表.md    ← 全量版（7 个 RWA 协议）
│   ├── 交易记录表.csv                 ← 全量版（41 步）
│   ├── 交易截图/                       ← 截图放这里（有命名规则）
│   ├── 已确认合约地址与链上实测.md      ← ⭐ 给中台的交付物（地址 / decimals / 解析规格）
│   ├── 如何获取交易Hash-实操教学.md     ← 查交易的方法 + 已捞样本（交叉验证/反例用）
│   ├── 实操清单-去哪里操作与需要提供什么.md  ← 产品页入口 + 域名警告（零成本路线已作废）
│   ├── 需要提供的交易Hash清单.md      ← 早期版本，参考
│   ├── 组合层风险与交叉依赖.md
│   ├── Plume-生态背景.md
│   └── 调研方法与信息源.md
...
### 🔴🔴 当前在做：优先 4 项实测（2026-08-03）

优先测 **① Solana-BNSOL ② Solana-Orca ③ ETH-Ember (Bitwise) ④ ETH-Re**。
Linnea 亲手操作 + 截图（统一放 `~/桌面/RWA/`），我事后补全解析。

- **主表（已合并旧仓库 + 链上验证）**：[03-参考/操作调研-全量汇总表.md](03-参考/操作调研-全量汇总表.md)
- 操作链接 + 逐步说明：[03-参考/优先4项-操作链接与记录表.md](03-参考/优先4项-操作链接与记录表.md)
- 记录表（23 步）：[03-参考/优先4项-交易记录表.csv](03-参考/优先4项-交易记录表.csv)

**已合并**：`linnea-l/DeFi-Protocol-Research/operation-research`（Huma / BNSOL / Ember，2026-07-29）
→ 每条 hash 已逐笔链上验证，纠出 3 处问题（Huma 三连交易性质、Ember hash 七链查不到、BNSOL 机制）

**最该抓到的三张截图**：
1. **Ember 存入后的持仓页**（Daily Processing 真空期，持仓是否显示 0）→ PRD 缺「申购中」状态的硬证据
2. **Re 的 reUSDe Redeem 面板**（页面到底显不显示季度赎回周期）→ 信息披露风险的证据
3. **Orca 页面的版本说法**（是 Whirlpools/CLMM 还是 V3）→ 确认解析对象没跑偏

### 🔴 全量任务（7 个 RWA 协议）：亲手把每种操作走一遍

Scott 指出：捞来的 hash **无法确认对应用户的哪一步操作**，而这正是我们要交付的东西。加上测试币没有额度 → **用主网小额亲手操作**。

每笔留下四件套：**操作步骤 + 交易类型 + tx hash + 截图**。

- 执行标准：[03-参考/交易采集-操作清单与记录表.md](03-参考/交易采集-操作清单与记录表.md)
- 直接填：[03-参考/交易记录表.csv](03-参考/交易记录表.csv)（已预填 41 行待办）
- 截图放：[03-参考/交易截图/](03-参考/交易截图/)（有命名规则）

**三个最该抓到的**：
1. **DMZ 每周派息** —— 第一天就要申购，否则第八天拿不到（PNL 公式关键）
2. **Ember 申购后的真空期** —— Daily Processing 期间持仓是否显示 0（PRD 缺「申购中」状态）
3. **Nest 的两种赎回** —— Instant 0.15% vs Standard 0.015%（后台单一字段装不下）

### 还缺的（不阻塞上面的主任务）

- **Re 的 NAV Oracle 地址** —— 唯一的硬阻塞
- Ember Premium+ 的 vault 合约地址（操作时顺手从 Details / Transparency tab 截下来）
- **OpenTrade 域名确认**（找 Cece，确认前不要连那个页面）
